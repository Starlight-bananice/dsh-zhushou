 ...
 */
import type { AssistantConfig, AssistantId, AssistantMemoryEntry, ChatId, ChatMessage, ChatSession, GlobalMemoryEntry, MemoryEntryId, ModelParams, SessionSelection, WorkspaceId } from './types.ts';
/** API 根路径（与 host 注册的 prefix 路由一致；不含尾部斜杠）。 */
export declare const API_BASE: "/assistant-panel/api";
/** 错误码（machine-readable；message 供展示）。 */
export type ApiErrorCode = 'BAD_REQUEST' | 'NOT_FOUND' | 'CONFLICT' | 'UNSUPPORTED' | 'LLM_ERROR' | 'INTERNAL' | 'ABORTED';
/** 失败响应中的 error 体。 */
 ...
 */
export type ApiRoute = 'GET    /api/health' | 'GET    /api/assistants' | 'POST   /api/assistants' | 'GET    /api/assistants/:id' | 'PUT    /api/assistants/:id' | 'DELETE /api/assistants/:id' | 'GET    /api/selection?sessionId=' | 'POST   /api/selection' | 'GET    /api/memory?assistantId=&global=' | 'POST   /api/memory' | 'PUT    /api/memory/:id' | 'DELETE /api/memory/:id' | 'GET    /api/skills?cwd=' | 'GET    /api/models' | 'GET    /api/workspaces' | 'GET    /api/profile' | 'PUT    /api/profile';
/** GET /api/health */
...
export type ListAssistantsResp = ApiEnvelope<{
...
export type CreateAssistantResp = ApiEnvelope<{
    assistant: AssistantConfig;
}>;
/** GET /api/assistants/:id */
export type GetAssistantResp = ApiEnvelope<{
    assistant: AssistantConfig;
}>;
...
export type UpdateAssistantResp = ApiEnvelope<{
    assistant: AssistantConfig;
}>;
/** DELETE /api/assistants/:id → 连带删除该助手私有记忆与快捷回复视图。 */
export type DeleteAssistantResp = ApiEnvelope<{
    id: AssistantId;
}>;
/**
...
export declare const CHAT_EVENT_TYPES: readonly ["connected", "text-delta", "reasoning-delta", "tool-call-delta", "memory-saved", "done", "error"];
/** @deprecated 同上（聊天 SSE 退役）。 */
...
/** @deprecated 聊天端点已退役。 */
export type ListChatsResp = ApiEnvelope<{
    chats: ChatSummary[];
}>;
/** @deprecated 聊天端点已退役。 */
export type GetChatMessagesResp = ApiEnvelope<{
    chat: ChatSession;
}>;
/** @deprecated 聊天端点已退役。 */
export type DeleteChatResp = ApiEnvelope<{
    id: ChatId;
}>;
/** GET /api/selection?sessionId=xxx → 当前会话选择状态（激活的助手 + 最近对话时间）。 */
export type GetSelectionResp = ApiEnvelope<{
...
export type SetSelectionResp = ApiEnvelope<{
    selection: SessionSelection;
}>;
...
export type CreateMemoryResp = ApiEnvelope<{
...
export type UpdateMemoryResp = ApiEnvelope<{
    entry: GlobalMemoryEntry | AssistantMemoryEntry;
}>;
/** DELETE /api/memory/:id */
export type DeleteMemoryResp = ApiEnvelope<{
    id: MemoryEntryId;
}>;
...
export type ListSkillsResp = ApiEnvelope<{
    skills: SkillInfo[];
}>;
...
export type ListModelsResp = ApiEnvelope<{
...
export type ListWorkspacesResp = ApiEnvelope<{
...
export type GetProfileResp = ApiEnvelope<{
...
export type UpdateProfileResp = ApiEnvelope<{
    profile: PluginProfile;
}>;
