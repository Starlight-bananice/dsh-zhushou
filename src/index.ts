...
export function apply(ctx: Context, config: Config): void {
  ...
  const tripwire = new Map<string, number>()

  /**
   * 包装 rebuilt 流：① reentrant 标记保持到迭代结束（finally 删除）；
   * ② 检测 error/aborted finish → 写熔断。
   */
  function guardRebuiltStream(sessionKey: string, source: AsyncIterable<StreamChunk>): AsyncIterable<StreamChunk> {
  ...
  ctx.on('llm/stream', (options: GenerateOptions, next) => {
    ...
    if (tripUntil !== undefined && tripUntil > Date.now()) return next()
