...
const REASONING_EFFORTS = ['auto', 'off', 'low', 'medium', 'high', 'max'] as const satisfies readonly ReasoningEffort[]
...
export function parseAssistantConfig(data: unknown): AssistantConfig {
  // 边界：schemastery 调用签名收 ObjectS 输入；unknown 在此显式收窄（校验由 schema 负责）
  return AssistantConfigSchema(data as any) as AssistantConfig
}

/** 校验创建输入 → 返回可落盘形状的 Omit 部分。 */
export function parseCreateInput(data: unknown): CreateAssistantInput {
  return CreateAssistantInputSchema(data as any) as CreateAssistantInput
}

/** 校验聊天消息。 */
export function parseChatMessage(data: unknown): ChatMessage {
  return ChatMessageSchema(data as any) as ChatMessage
}
